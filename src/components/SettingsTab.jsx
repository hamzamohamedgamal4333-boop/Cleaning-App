            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm space-y-4">
              <div className="flex items-center gap-2 border-b border-slate-100 pb-3">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-400">حفظ أو استعادة كافة بيانات المتجر والمزامنة السحابية</p>
                </div>
              </div>
              ...
              <div className="p-4 bg-slate-50 border border-slate-200 rounded-2xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                  <div className={`p-2.5 rounded-xl text-white ${isSupabaseConfigured() ? "bg-teal-600 shadow-md shadow-teal-100" : "bg-amber-500 shadow-md"}`}>
                    <Cloud size={20} />
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="font-black text-slate-800 text-xs sm:text-sm">حالة السحابة:</span>
                      {isSupabaseConfigured() ? (
                        <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-emerald-100 text-emerald-800 text-[11px] font-black border border-emerald-200">
                          <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
                          <span>متصل بالسحابة (Supabase Live)</span>
                        </span>
                      ) : (
                        <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-amber-100 text-amber-800 text-[11px] font-black border border-amber-200">
                          ...
                          <span>وضع العمل المحلي (Offline / Local Mode)</span>
                        </span>
                      )}
                    </div>
                    <p className="text-[11px] text-slate-500 font-semibold mt-1">
                      {isSupabaseConfigured()
                        ? "المزامنة المباشرة شغالة لجميع عمليات البيع والمخزون والحسابات عبر كافة الأجهزة."
                        : "لم يتم ضبط مفاتيح VITE_SUPABASE_URL. يعمل النظام محلياً لضمان عدم توقف المتجر."}
                    </p>
                  </div>
                </div>
                ...
                <button
                  type="button"
                  onClick={() => {
                    if (onRefreshCloudData) onRefreshCloudData();
                    showNotification("تمت إعادة مزامنة وتحديث بيانات المتجر من السحابة بنجاح!");
                  }}
                  className="px-3.5 py-2 bg-white hover:bg-teal-50 text-teal-700 border border-slate-200 hover:border-teal-300 font-bold text-xs rounded-xl shadow-xs transition-all flex items-center justify-center gap-1.5 cursor-pointer shrink-0"
                >
